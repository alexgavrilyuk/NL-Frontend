// src/features/shared/layout/Sidebar.js
...
const Sidebar = () => {
  ...
  return (
    <aside
      className={`bg-background-secondary text-text-primary border-r border-border-light transition-all duration-300 ease-in-out ${
        collapsed ? 'w-16' : 'w-64'
      }`}
    >
      <div className="p-md flex justify-between items-center">
        {!collapsed && (
          <span className="text-primary-500 font-bold text-lg">NeuroLedger</span>
        )}
        <button
          ...
          {navigationItems.map((item) => (
            <li key={item.path}>
              <NavLink
                to={item.path}
                className={({ isActive }) => `
                  flex items-center px-md py-sm hover:bg-background-tertiary
                  ${isActive ? 'bg-primary-500 bg-opacity-10 text-primary-500 border-r-4 border-primary-500' : ''}
                `}
              >
                <span className="inline-block">{item.icon}</span>
                {!collapsed && <span className="ml-sm">{item.name}</span>}
              </NavLink>
            </li>
          ))}
        </ul>
